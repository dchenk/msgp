...
// ReadFile reads a file into dst using a read-only memory mapping. The file must be mmap-able,
// and the Unmarshaler should never write to the source memory. (Methods generated by the msgp
// tool obey that constraint, but user-defined implementations may not.)
//
// Reading and writing through file mappings is only efficient for large files; small files are
// best read and written using the ordinary streaming interfaces.
